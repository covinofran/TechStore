...
function renderCart() {
    ...
    cart.forEach((item) => {
        console.log("Imagen en renderCart:", item.image); // Depuración

        total += item.price * item.quantity;
        const productElement = document.createElement("div");
        productElement.className = "d-flex align-items-center bg-white shadow-sm rounded-lg p-3 mb-3";

        const imageSrc = item.image || "assets/images/default-placeholder.png";
        productElement.innerHTML = `
            <div class="d-flex align-items-center flex-grow-1">
                <img src="${imageSrc}" alt="${item.title}" class="rounded me-3" style="width: 60px; height: 60px; object-fit: cover;">
                <div>
                    <h5 class="mb-1 text-gray-800 fw-bold">${item.title}</h5>
                    <p class="mb-0 text-sm text-gray-600">Precio: $${item.price.toFixed(2)}</p>
                    <p class="mb-0 text-sm text-gray-600">
                        Cantidad: 
                        <input type="number" min="1" value="${item.quantity}" class="form-control form-control-sm d-inline w-25 ms-2">
                    </p>
                </div>
            </div>
            <div class="text-end">
                <p class="fw-bold text-gray-900">$${(item.price * item.quantity).toFixed(2)}</p>
...
function addToCart(productId, title, price, imageUrl) {
    const cart = JSON.parse(localStorage.getItem("cart")) || [];
    const validImageUrl =
        imageUrl && imageUrl.trim() !== "" ? imageUrl : "assets/images/default-placeholder.png";
